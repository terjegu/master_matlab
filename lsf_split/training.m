function [gmm,index_all] = training(X_lsf,Y_lsf,m)
...
N = numel(X_lsf(:,1));          % Number of training vectors
p = numel(X_lsf(1,:));          % Number of parameters
...
%     disp(index_z);
    ...
    S.PComponents = NaN(m,1);           % Prior of each cluster
    for j=1:m
        S.Sigma(:,:,j) = cov(Z_t(J==j,:));
        S.PComponents(j) = sum(J==j)/N_j;
    end
