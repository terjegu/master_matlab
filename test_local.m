...
%         for k=1:N_lab
%             % Error difference
% %             error_sec(k) = abs(w_p_x(k,1)-w_p_w(k));
%             if abs(w_p_x(k,1)-w_p_w(k))/fs > error_diff_bound
%                 error_diff = error_diff + 1;
%             end    
%         end
%         error_sec = mean(error_sec);

        % Error swap
        for k=2:N_lab-1
            if (w_p_w(k) > w_p_x(k+1,1)) || (w_p_w(k) < w_p_x(k-1,1))
        ...
        if error_swap < error_best
           error_best = error_swap;
           kk1_f = i;
           kk3_f = j;
        end 
    end
